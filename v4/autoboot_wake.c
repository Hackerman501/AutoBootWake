...
static wake_t detect_wake_reason() {
    // WUT exposes no reliable wake source yet.
    // This is a safe stub until MCP/IOS hooks are added.
    return WAKE_OTHER;
}
...
static const char *select_boot_target(wake_t w) {
    if (w == WAKE_POWER && strlen(g_cfg.boot_power))
        return g_cfg.boot_power;
...
int plugin_main() {
    ...
    wake_t w = detect_wake_reason();

    if (w == WAKE_POWER) log_line("abw: wake=power\n");
    else if (w == WAKE_GAMEPAD) log_line("abw: wake=gamepad\n");
    else log_line("abw: wake=other\n");

    OSSleepTicks(OSSecondsToTicks(g_cfg.timeout));

    const char *target = select_boot_target(w);

    char out[300];
    snprintf(out, sizeof(out), "abw: boot -> %s\n", target);
    log_line(out);

    /* Real chainload comes later */
    return 0;
}
